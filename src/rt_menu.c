...
typedef enum
{
    ...
    USEPORT2_CLASSIC,
    PADENABLE_CLASSIC,
    SPACEBALLENABLE_CLASSIC,
    CYBERMANENABLE_CLASSIC,
    THRESSENS_CLASSIC,
    MOUSESENS_CLASSIC,
    CUSTOMIZE_CLASSIC
} controltypes_classic;
...
CP_iteminfo CtlItems  = {CTL_X, MENU_Y, 5, -1, 36, CtlMenuNames, mn_largefont};
CP_itemtype CtlMenu[] =
{
...
CP_MenuNames CtlMenuNames_Classic[] =
{
    "MOUSE ENABLED",
    "JOYSTICK ENABLED",
    "USE JOYSTICK PORT 2",
...
CP_iteminfo CtlItems_Classic  = { CTL_X, MENU_Y, 9, -1, 36, CtlMenuNames_Classic, mn_largefont };
CP_itemtype CtlMenu_Classic[] =
{
    { CP_Inactive, "ctl_mic\0", 'M', NULL },
    { CP_Inactive, "ctl_jen\0", 'J', NULL },
...
CP_iteminfo RTLItems = {32, 48, 5, 0, 32, RTLMenuNames, mn_largefont };
...
CP_iteminfo ControlMItems = {32, 40, 6, 0, 32, ControlMMenuNames, mn_largefont};

CP_itemtype ControlMMenu[] =
{
    {2, "cntl\0",       'C', (menuptr)CP_Control},
    {1, "uopt\0",       'U', (menuptr)CP_OptionsMenu},
    {1, "vopt\0",       'V', (menuptr)CP_VideoMenu},
    {1, "bopt\0",       'B', NULL}, // (menuptr)CP_BonusMenu},
    {1, "muvolumn\0",   'M', (menuptr)MusicVolume},
    {1, "fxvolumn\0",   'S', (menuptr)FXVolume}
};

CP_MenuNames ControlMMenuNames_Classic[] =
{
    "CONTROLS",
    "USER OPTIONS",
    "MUSIC VOLUME",
...
CP_iteminfo ControlMItems_Classic = {32, 48, 4, 0, 32, ControlMMenuNames_Classic, mn_largefont};

CP_itemtype ControlMMenu_Classic[] =
{
    {2, "cntl\0",       'C', (menuptr)CP_Control},
    {1, "uopt\0",       'U', (menuptr)CP_OptionsMenu},
    {1, "muvolumn\0",   'M', (menuptr)MusicVolume},
...
CP_MenuNames OptionsNames_Classic[] =
{
    "AUTO DETAIL ADJUST",
    "LIGHT DIMINISHING",
    "BOBBIN'",
...
CP_iteminfo OptionsItems_Classic = { 20, MENU_Y, 9, 0, 43, OptionsNames_Classic, mn_largefont };

CP_itemtype OptionsMenu_Classic[] =
{
    {2, "autoadj\0", 'A', NULL},
    {1, "lightdim\0",'L', NULL},
    {1, "bobbin\0",  'B', NULL},
...
CP_iteminfo VideoItems = { 20, MENU_Y, 4, 0, 43, VideoNames, mn_largefont };

CP_itemtype VideoMenu[] =
{
    {2, "", 'R', (menuptr)CP_VideoResMenu},
    {1, "", 'P', (menuptr)CP_AspectMenu},
    {1, "", 'F', NULL},
    {1, "", 'C', NULL}
};

CP_MenuNames AspectMenuNames[] =
{
    "ON",
    "OFF",
    "OFF (STRETCHED)"
...
CP_iteminfo AspectItems = { 32, 64, 3, 0, 43, AspectMenuNames, mn_largefont };

CP_itemtype AspectMenu[] =
{
    {2, "", 'N', NULL},
    {1, "", 'F', NULL},
    {1, "",  'S', NULL}
...
void ScanForSavedGames (int rtl)
{
    struct find_t f;
    char filename[256];
    char str[45];
...
#else
        strncpy (filename, SaveFileName, 256);
    ...
    {
        GenerateNewRTLBasedFileName(rtl, RTLBASED_SAVENAME);

        NewSaveFileName[7]='?';
#if PLATFORM_WIN32
        GetPathFromEnvironment( filename, ApogeePath, NewSaveFileName );
#else
        strncpy (filename, NewSaveFileName, 256);
        pathsave = getcwd (NULL, 0);
        chdir (ApogeePath);
#endif
    }
...
void SetUpControlPanel (void)
{
    ...
    for (i = 0; i < 320; i += 2)
    {
        ...
        for (j = 0; j < 100; j++, s++, b += (screen_src_width << 1)) // (320 << 1))
            *s = *b;
    }

    if (pure || enabledrtls == 0)
        ScanForSavedGames (currentrtl);
...
void CleanUpControlPanel (void)
{
    ...
    INL_GetJoyDelta (JOY_TEMP, &joyx, &joyy);
...
void DrawOrderInfo (int which)
{
    ...
    if (lumpname[0] == 'S')
    {
        ...
        VL_DrawPic(screen_src_statusbar_x, 0, (pic_t *)W_CacheLumpNum(start + which, PU_CACHE, Cvt_pic_t, 1));
    }
    else
    {
        VL_DrawPostPic(W_GetNumForName("trilogo"));
...
void CP_ViewScores (void)
{
    ...
    CheckHighScore(0, 0, true, chosenmenurtl);
}
...
void EndGameStuff (void)
{
    ...
    pickquick = false;
    CheckHighScore (gamestate.score, gamestate.mapon+1, true, currentrtl);
...

{
    ...
    if (!pure && enabledrtls != 0 && currentrtl != chosenmenurtl)
    {
        quicksaveslot = -1;
        currentrtl = chosenmenurtl;
    }
...
int DoLoad (int which, int rtl)
{
    ...
    if ((which >= 0) && SaveGamesAvail[which])
    {
        ...
        if (LoadTheGame (which, &game, rtl) == true)
        {
            MenuFixup ();
            StartGame = true;
            exit      = 1;
...
int CP_LoadGame (int quick, int dieload)
{
    ...
    else if (!pure && enabledrtls != 0)
    {
        ...
        MenuNum = 6;
        ScanForSavedGames(chosenmenurtl);
    }
    ...
    do
    {
        ...
        if (!pure && enabledrtls != 0 && !quick)
        {
            if ((exit = DoLoad (which, chosenmenurtl)))
                break;
        }
        else
        {
            if ((exit = DoLoad (which, currentrtl)))
                break;
        }
...
void QuickSaveGame (void)
{
    ...
    sprintf(numHex, "%1x", quicksaveslot);
    ...
    {
        b=(byte *)bufferofs+i;
        for (j=0; j<100; j++,s++,b+=(screen_src_width<<1))
            *s=*b;
    }
...
#define CTL_SPC   70
    ...
    if (pure)
    {
        do
        {
            which = HandleMenu (&CtlItems_Classic, &CtlMenu_Classic[0], NULL);

            switch (CSTactive)
            {
                case MOUSEENABLE_CLASSIC:
                    if (MousePresent)
                    {
                        mouseenabled ^= 1;
                        DrawCtlButtons();
                ...
                case THRESSENS_CLASSIC:
                case MOUSESENS_CLASSIC:
                case CUSTOMIZE_CLASSIC:
                    DrawCtlScreen ();
                    break;
            }
        }
...
#define CALY   22
    ...
    else
    {
        ...
        checkbits = ( 1 << 0 ) + ( 1 << 1 );
        // if (joystickport) checkbits = ( 1 << 2 ) + ( 1 << 3 );
    }
    ...
    while( !done )
    {
        ...
        IN_GetJoyAbs( JOY_TEMP, &xmin, &ymin );
        MN_PlayMenuSnd( SD_SELECTSND );
...
//      DrawMenuBufItem( CALX, CALY, W_GetNumForName( "joystk1" ) );
        ...
        IN_GetJoyAbs( JOY_TEMP, &xmax, &ymax );
        MN_PlayMenuSnd( SD_SELECTSND );
        ...
        {
            IN_SetupJoy( JOY_TEMP, xmin, xmax, ymin, ymax );
            joyxmin = xmin;
            joyxmax = xmax;
            joyymin = ymin;
            joyymax = ymax;
...
void DrawCtlScreen (void)
{
    ...
    if (pure)
    {
        DrawMenu (&CtlItems_Classic, &CtlMenu_Classic[0]);
        DrawMenuBufItem (CtlItems_Classic.x, ((CtlItems_Classic.curpos*14)+(CtlItems_Classic.y-2)),
                         W_GetNumForName( LargeCursor ) + CursorFrame[ CursorNum ] );
    }
    else
    {
...
void DrawCtlButtons (void)
{
    ...
    if (pure)
    {
        if (first)
        {
            if (MousePresent)
            {
                CtlMenu_Classic[THRESSENS_CLASSIC].active = CP_Active;
                CtlMenu_Classic[MOUSESENS_CLASSIC].active = CP_Active;
                CtlMenu_Classic[MOUSEENABLE_CLASSIC].active = CP_Active;
            }
            else
            {
                CtlMenu_Classic[MOUSEENABLE_CLASSIC].active = CP_Inactive;
                mouseenabled = 0;
            }

            for (x = 0; x < CtlItems_Classic.amount; x++)
            {
                if (CtlMenu_Classic[x].active)
                {
                    CtlMenu_Classic[x].active = CP_CursorLocation;
                    break;
                }
            }
            first = false;
        ...
        x = CTL_X+CtlItems_Classic.indent-18;
        y = MENU_Y-1;
        ...
        if ((CtlItems_Classic.curpos < 0) || (!CtlMenu_Classic[CtlItems_Classic.curpos].active))
            for (i = 0; i < CtlItems_Classic.amount; i++)
                if (CtlMenu_Classic[i].active)
                {
                    CtlItems_Classic.curpos = i;
                    break;
                }
    }
    else
...
//   struct Spw_IntPacket packet;
    ...
    if (joystickenabled && !mouseactive)
    {
        ...
        INL_GetJoyDelta (JOY_TEMP, &jx, &jy);
...
void DrawControlMenu (void)
{
    ...
    if (pure)
    {
        MN_GetCursorLocation( &ControlMItems_Classic, &ControlMMenu_Classic[ 0 ] );
        DrawMenu (&ControlMItems_Classic, &ControlMMenu_Classic[0]);
    }
    else
    {
...
void CP_ControlMenu (void)
{
    ...
    if (pure)
    {
        do
        {
            which = HandleMenu (&ControlMItems_Classic, &ControlMMenu_Classic[0], NULL);
        }
        while (which >= 0);
    }
    else
...
void DrawOptionsMenu (void)
{
    ...
    if (pure)
    {
        MN_GetCursorLocation( &OptionsItems_Classic, &OptionsMenu_Classic[ 0 ] );
        DrawMenu (&OptionsItems_Classic, &OptionsMenu_Classic[0]);
    }
    else
    {
...
void DrawVideoMenu (void)
{
    ...
    MN_GetCursorLocation( &VideoItems, &VideoMenu[ 0 ] );
    DrawMenu (&VideoItems, &VideoMenu[0]);
    DrawVideoButtons ();
    DisplayInfo (0);
...
void CP_VideoMenu (void)
{
    ...
    do
    {
        which = HandleMenu (&VideoItems, &VideoMenu[0], NULL);
...
void DrawVideoButtons (void)
{
    ...
    for (i = 2; i < VideoItems.amount; i++)
        if (VideoMenu[i].active != CP_Active3)
        {
            //
            ...
            {
                case 2:
                    if (screen_fullscreen == 1) on = 1;
                    break;
                case 3:
                    if (screen_classic == 1) on = 1;
                    break;
            }

            if (on)
                DrawMenuBufItem (20+22, VideoItems.y+i*14-1, button_on);
            else
                DrawMenuBufItem (20+22, VideoItems.y+i*14-1, button_off);
        }
}

void CP_VideoResRedraw (void)
{
    SetMenuHeader(screen_dest_resolution_strings[screen_dest_resolution_index]);
}

void VideoResMenuUpdate (int which)
{
    if (which != screen_dest_resolution_index)
    {
        screen_dest_resolution_index = which;
        ChangeScreenResolution(screen_dest_resolution[which*2],
                               screen_dest_resolution[(which*2)+1]);
        ChangeVideoMode();
        VL_SetVGAPlaneMode();
        VL_SetPalette(origpal);
        ShutdownMenuBuf();
        SetupMenuBuf();
        DrawMultiPageCustomMenu("Video Resolution", CP_VideoResRedraw);
    }
}

void CP_VideoResMenu (void)
{
    HandleMultiPageCustomMenu(screen_dest_resolution_strings,
                              screen_dest_resolution_size, screen_dest_resolution_index,
                              "Video Resolution", VideoResMenuUpdate, CP_VideoResRedraw, false);
    handlewhich = 100;
    DrawVideoMenu();
}

void DrawAspectMenu (void)
{
    MenuNum = 1;

    SetAlternateMenuBuf();
    ClearMenuBuf();
    SetMenuTitle ("Correct Pixel Ratio?");

    MN_DrawButtons (&AspectItems, &AspectMenu[0], 1 - screen_aspect, OptionNums);
    MN_GetCursorLocation( &AspectItems, &AspectMenu[ 0 ] );
    DrawMenu (&AspectItems, &AspectMenu[0]);
    DisplayInfo (0);
    FlipMenuBuf();
}

void AspectMenuUpdate()
{
    ChangeScreenResolution(screen_dest_width, screen_dest_height);
    ChangeVideoMode();
    VL_SetVGAPlaneMode();
...
void CP_AspectMenu (void)
{
    ...
    do
    {
        which = HandleMenu (&AspectItems, &AspectMenu[0], NULL);
...
void CP_OptionsMenu (void)
{
    ...
    if (pure)
    {
        do
        {
            which = HandleMenu (&OptionsItems_Classic, &OptionsMenu_Classic[0], NULL);
...
void DrawOptionsButtons (void)
{
    ...
    if (pure)
    {
        for (i = 0; i < OptionsItems_Classic.amount-5; i++)
            if (OptionsMenu_Classic[i].active != CP_Active3)
            {
                //
                // DRAW SELECTED/NOT SELECTED GRAPHIC BUTTONS
                //
                ...
                if (on)
                    DrawMenuBufItem (20+22, OptionsItems_Classic.y+i*14-1, button_on);
                else
                    DrawMenuBufItem (20+22, OptionsItems_Classic.y+i*14-1, button_off);
            }
    }
    else
    {
        for (i = 0; i < OptionsItems.amount-4; i++)
...
void DrawRTLMenu (void)
{
    MenuNum = 1;

    if (chosenmenurtl == RTL_CUSTOM)
        MN_MakeActive( &RTLItems, &RTLMenu[0], 4 );
    else
        MN_MakeActive( &RTLItems, &RTLMenu[0], chosenmenurtl );

    if (!(enabledrtls & 1 << RTL_STANDARD))
        RTLMenu[RTL_STANDARD].active = CP_Inactive;

    if (!(enabledrtls & 1 << RTL_SHAREWARE))
        RTLMenu[RTL_SHAREWARE].active = CP_Inactive;

    if (!(enabledrtls & 1 << RTL_EXTREME))
        RTLMenu[RTL_EXTREME].active = CP_Inactive;

    RTLMenu[3].active = CP_Inactive;
    RTLMenu[4].active = CP_SemiActive;

    if (numdone || (!ingame) || (!inmenu))
        SetAlternateMenuBuf();
    ClearMenuBuf();
    SetMenuTitle ("Choose Game Levels");

    DrawMenu (&RTLItems, &RTLMenu[0]);
...
int CP_RTLSelection (void)
{
    ...
    do
    {
        which = HandleMenu (&RTLItems, &RTLMenu[0], NULL);
        ...
        if (which == 4)
        {
            CP_ErrorMsg("Choose Game Levels",
                        "This feature is currently not supported. "
                        "Please use the FILERTL argument for now.",
                        mn_smallfont);

            DrawRTLMenu();
        }
    }
    while (RTLMenu[which].active == CP_Inactive || RTLMenu[which].active == CP_SemiActive );
...
void CP_ModemGameMessage (int player)
{
    ...
    else
    {
        ...
        if (networkgame==true && gamestate.Product != ROTT_SITELICENSE)
        {
...
void ShowBattleOption (boolean inmenu, int PosX, int PosY, int column,
                       int Line, char *text1, char *text2)
{
    char text[80];
    int x;
    int y;
    ...
    {
        y = PosY + (Line * 7 * screen_src_minscale);
        x = PosX + (column * 120 * screen_src_minscale);
    }
    ...
    if (inmenu)
        ...
        PrintBattleOption(inmenu, x + (60 * screen_src_minscale), y, text);
}
...
void ShowBattleOptions (boolean inmenu, int PosX, int PosY)
{
    ...
    if (inmenu)
        ...
        temp = screen_src_width;

    if (inmenu)
        PrintBattleOption(inmenu, (temp - width) / 2, PosY, text);
    else
        PrintBattleOption(inmenu, (temp - (width * screen_src_minscale)) / 2, PosY, text);
...

{
    ...
    if (BATTLEMODE || pure)
        ...
        GetMapInfoForRTL(mapinfo, currentrtl);
