...
void ScaleFilmPost (byte *src, byte *buf)
{
    ...
    for (; offset!=255;)
    {
        ...
        if (cin_yh >= screen_src_height)
            cin_yh = screen_src_height-1;
        if (cin_yl < 0)
            cin_yl = 0;
        if (cin_yl <= cin_yh)
        {
...
void DrawCinematicBackground ( backevent *back )
{
    ...
    height = pic->height;
    if (height+back->yoffset>200)
        height=200-back->yoffset;
...
//
    ...
    if (x1<0)
    {
        ...
        frac=0;
    x2 = x2 >= screen_src_width ? (screen_src_width-1) : x2;
...
=================
=
= DrawCustomCinematicSprite
=
=================
*/
void DrawCustomCinematicSprite (int x, int y, int scale, int shapenum)
{
    byte   *shape;
    int    frac;
    patch_t *p;
    ...
    cin_ycenter=y >> FRACTIONBITS;
    cin_invscale = (height<<FRACTIONBITS)/p->origsize;
    buf=(byte *)bufferofs;
    tx=-p->leftoffset;
    xcent=(x & 0xffff0000)-(height<<(FRACTIONBITS-1))+(FRACTIONUNIT>>1);
...
void DrawClearBuffer ( void )
{
    memset((byte *)bufferofs,0,320*200);
}
...
{
    byte *src, *dest, *origdest;
    lpic_t *pic;
    int width, height, ht, origwidth, origheight, hstart;
    int i, j;
    double cur;

    pic = (lpic_t *)W_CacheLumpNum(lumpnum, PU_CACHE, Cvt_lpic_t, 1);
    width = origwidth = 320;
    ht = origheight = pic->height;

    origdest = (byte *)bufferofs;
    dest = origdest;
    src = &(pic->data);

    ht = origheight * screen_src_scalecropratio;

    hstart = (origheight - ht) >> 1;
    ...
    while (width--)
    {
        ...
        while (height--)
        {
            for (i = 0; i < xscalelookupmax[width]; i++)
                for (j = 0; j < yscalelookupmax[height]; j++)
                    *(dest + i + (screen_src_width * j)) = *src;

            src++;
            dest += screen_src_width * yscalelookupmax[height];
        }

        src += origheight - ht - hstart;

        origdest += xscalelookupmax[width];
        dest = origdest;
    }
}
