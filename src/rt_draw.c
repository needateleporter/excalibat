...
static const int weaponshapesw[NUMWEAPGRAPHICS_SW] =
{
    W_MALEPISTOL1 - (W_MALEPISTOL1 - W_KNIFE),
    W_MRIGHTPISTOL1 - (W_MALEPISTOL1 - W_KNIFE),
    W_MP40 - (W_MALEPISTOL1 - W_KNIFE) - (W_MP40 - W_FEMALEPISTOL1),
    W_BAZOOKA - (W_MALEPISTOL1 - W_KNIFE) - (W_MP40 - W_FEMALEPISTOL1),
    W_HEATSEEKER - (W_MALEPISTOL1 - W_KNIFE) - (W_MP40 - W_FEMALEPISTOL1),
    W_DRUNK - (W_MALEPISTOL1 - W_KNIFE) - (W_MP40 - W_FEMALEPISTOL1),
    W_FIREBOMB - (W_MALEPISTOL1 - W_KNIFE) - (W_MP40 - W_FEMALEPISTOL1),
    W_FIREWALL - (W_MALEPISTOL1 - W_KNIFE) - (W_MP40 - W_FEMALEPISTOL1),
    W_GODHAND - (W_MALEPISTOL1 - W_KNIFE) - (W_MP40 - W_FEMALEPISTOL1)
};
...
void DrawPlayerWeapon (void)
{
    ...
    if (((locplayerstate->NETCAPTURED >= 1) || (locplayerstate->NETCAPTURED == -2))
    ...
    else if (locplayerstate->weapon != wp_twopistol)
    {
        ...
        if ((index < 0) || (index >= NUMWEAPGRAPHICS) ||
                (shareware && (index >= NUMWEAPGRAPHICS_SW)))
            Error ("Weapon shapenum out of range\n");

        if (!shareware)
            shapenum = gunsstart + weaponshape[index] + locplayerstate->weaponframe;
        else
            shapenum = gunsstart + weaponshapesw[index] + locplayerstate->weaponframe;
    ...
    else
    {
        ...
        {
            altshape = W_MLEFTPISTOL1 - (W_MALEPISTOL1 - W_KNIFE);
            shapenum = W_MRIGHTPISTOL1 - (W_MALEPISTOL1 - W_KNIFE);
        }
        else
        {
...
void SetSpriteLightLevel (int x, int y, visobj_t *sprite, int dir, int fullbright)
{
    ...
    if (fog)
    {
        i = ((int)(sprite->viewheight / screen_src_hscale) >> normalshade) + minshade;
    ...
    else
    {
        if (lightsource)
        {
            ...
            i = maxshade - ((int)(sprite->viewheight / screen_src_hscale) >> normalshade) - lv;
        ...
        {
            i = maxshade - ((int)(sprite->viewheight / screen_src_hscale) >> normalshade);
...
void SetColorLightLevel (int x, int y, visobj_t *sprite, int dir, int color, int fullbright)
{
    ...
    else
    {
        if (lightsource)
        {
            ...
            lv = (((LightSourceAt(x >> 16, y >> 16) >> intercept) & 0xf) >> 1);
            i = maxshade - ((int)(height / screen_src_hscale) >> normalshade) - lv;
...
void SetWallLightLevel (wallcast_t *post)
{
    ...
    if (fog)
    {
        i = ((int)(post->wallheight / screen_src_hscale) >> normalshade) + minshade - lv + la;
    ...
    {
        i = maxshade - ((int)(post->wallheight / screen_src_hscale) >> normalshade) - lv + la;
...
// Set YZ angle
    ...
    if (yzangle>ANG180)
    {
        centery-=FixedMul(FINEANGLES-yzangle,yzangleconverter) * screen_src_hscale;
        centeryunscaled-=FixedMul(FINEANGLES-yzangle,yzangleconverter);
    }
    else
    {
        centery+=FixedMul(yzangle,yzangleconverter) * screen_src_hscale;
        centeryunscaled+=FixedMul(yzangle,yzangleconverter);
    }
...
//    int     plane;
    ...
    xsize=(screen_src_width<<16)/step;
    if (xsize>screen_src_width) xsize=screen_src_width;
    ysize=(screen_src_height<<16)/step;
    if (ysize>screen_src_height) ysize=screen_src_height;

    {
        yfrac=0;
        for (j=y; j<y+ysize; j++)
        {
            p=src+(screen_src_width*(yfrac>>16));
...
void DoLoadGameSequence()
{
    ...
    // Use the regular fizzle-in sequence for higher
    // resolutions, so we don't have to worry about
    // non-4:3 resolutions.
    if (!screen_classic)
    {
        fizzlein = true;
...
void StartupRotateBuffer (int masked)
{
    ...
    if (RotateBufferStarted)
        return;

    RotateBufferStarted = true;

    //memsize = 512 * 256 * screen_src_wscale * screen_src_hscale;
    memsize = 131072;
    RotatedImage = SafeMalloc(memsize);

    if (masked == 0)
        memset(RotatedImage, 0, memsize);
    else
        memset(RotatedImage, 0xff, memsize);

    for (a = 0; a < 200; a++)
        for (b = 0; b < 320; b++)
            *(RotatedImage + 99 + ((a + 28) << 9) + b) = *((byte *)bufferofs + (a * screen_src_width) + b);
}
...
void ScaleAndRotateBuffer (int startangle, int endangle, int startscale, int endscale, int time)
{
    ...
    for (i = 0; i < time; i += tics)
    {
        ...
        {
            screen_zoom_rot = -((double)((angle >> 16) & (FINEANGLES - 1)) / FINEANGLES) * 360;
            screen_zoom_scale = (double)(scale >> 6) / FINEANGLES;
        }
        FlipPage();
    ...
    {
        screen_zoom_rot = -((double)(endangle & (FINEANGLES - 1)) / FINEANGLES) * 360;
        screen_zoom_scale = (double)endscale / FINEANGLES;
    }
...
void DrawRotatedScreen (int cx, int cy, byte *destscreen, int angle, int scale, int masked)
{
    ...
    if (masked == 0)
        ...
        for (y = 0; y < screen_src_height; y++, mr_xfrac += c, mr_yfrac -= s)
            DrawMaskedRotRow(screen_src_width, screen + ylookup[y], RotatedImage);
}
...
void ApogeeTitle (void)
{
    ...
    int APOGEEXANGLE = 913;
    int APOGEEXMAG = 180 * screen_src_wscale;
    int APOGEESTARTY = 0;
    int APOGEEENDY = screen_src_height / 2.0;
    int APOGEESCALESTART = (FINEANGLES << 4) / screen_src_minscale;
    int APOGEESCALEEND = FINEANGLES / screen_src_minscale;
    int APOGEESONGTIME = 124 - 1;
    ...
    DrawNormalSpriteUnscaled(0, 0, W_GetNumForName("ap_titl"));
    ...
    while (time >= 0)
    {
        ...
        x = (100 * screen_src_wscale) + FixedMul(APOGEEXMAG, sintable[anglex>>16]);
...
void RotationFun (void)
{
    ...
    while (!Keyboard[sc_Escape])
    {
        ...
        {
            screen_zoom_rot = -((double)angle / FINEANGLES) * 360;
            screen_zoom_scale = (double)scale / FINEANGLES;
        }
    ...
    Keyboard[sc_Escape] = 0; // Doesn't actually work.
...
void SetupScreenSaverPhase ( void )
{
    ...
    {
        ScreenSaver->x=(int)(160 * screen_src_wscale);
        ScreenSaver->y=(int)(100 * screen_src_hscale);
        ScreenSaver->angle=0;
...
void StartupScreenSaver ( void )
{
    ...
    ScreenSaver->pausex=(int)(120 * screen_src_wscale);
    ScreenSaver->pausey=(int)(84 * screen_src_hscale);
    SetupScreenSaverPhase();
}
...
void UpdateScreenSaver ( void )
{
    ...
    else if (ScreenSaver->x>screen_src_width-SPINSIZE)
    {
        ScreenSaver->x=screen_src_width-SPINSIZE;
        ScreenSaver->dx=-(abs(ScreenSaver->dx));
        ScreenSaver->dy+=(RandomNumber("Rotate",0)>>6)-2;
    }
    ...
    else if (ScreenSaver->y>screen_src_height-SPINSIZE)
    {
        ScreenSaver->y=screen_src_height-SPINSIZE;
        ScreenSaver->dy=-(abs(ScreenSaver->dy));
        ScreenSaver->dx+=(RandomNumber("Rotate",0)>>6)-2;
    }
    ...
    {
        ScreenSaver->pausetime=PAUSETIME;
        ScreenSaver->pausex=RandomNumber ("pausex",0)%(240 * (int)screen_src_hscale);
        ScreenSaver->pausey=RandomNumber ("pausey",0)%(168 * (int)screen_src_hscale);
...
{
    memcpy((byte *)bufferofs, bkgnd, screensize);
}
...
{
    memcpy(bkgnd, (byte *)bufferofs, screensize);
}
...
void WarpSprite (int x, int y, int endx, int endy, int time, byte *back, int shape)
{
    int dx;
    int dy;
    int cx;
...
void DoEndCinematicShareware (void)
{
    int trilogo;
    int group;
    int world;
    ...
    WarpSprite((int)(screen_src_width / 2), (int)(-screen_src_height / 2),
               (int)(screen_src_width / 2), (int)(screen_src_height / 2),
               (VBLCOUNTER*3), bkgnd, W_GetNumForName("youwin"));
    if (LastScan != 0)
        goto fadelogo;
...
fadelogo:
    ...
    WarpSprite((int)(screen_src_width / 2), screen_src_height + (int)(50 * screen_src_minscale),
               (int)(screen_src_width / 2), (int)(screen_src_height / 2),
               VBLCOUNTER * 3, bkgnd, W_GetNumForName("wrldsafe"));
    if (LastScan !=0)
        goto fadeworld;
    ...
    WarpSprite((int)(screen_src_width / 2), (int)(screen_src_height / 2),
               (int)(screen_src_width / 2), (int)(-50 * screen_src_minscale),
               VBLCOUNTER * 3, bkgnd, W_GetNumForName("wrldsafe"));
    if (LastScan !=0)
        goto fadeworld;
...
fadeworld:
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        tx = (32 * screen_src_minscale) + ((int)(i * screen_src_minscale) << 6);
        ty = screen_src_height >> 1;
    ...
    for (i = 0; i < NUMENDMESSAGES; i++)
    {
        ...
        x = (screen_src_width - (int)(width * screen_src_minscale)) >> 1;
        y = (screen_src_height - (int)(height * screen_src_minscale)) >> 1;
        time1 = (screen_src_height + (int)(100 * screen_src_minscale) - y) * (VBLCOUNTER * 4) /
                (screen_src_height + (int)(100 * screen_src_minscale));
        time2 = VBLCOUNTER * 4 - time1;

        WarpString(x, screen_src_height + (int)(50 * screen_src_minscale),
                   x, y - (int)(50 * screen_src_minscale), time1, bkgnd, EndCinematicText[i]);
        if (LastScan != 0)
            break;
        ...
        WarpString(x, y - (int)(50 * screen_src_minscale),
                   x, (int)(-50 * screen_src_minscale), time2, bkgnd, EndCinematicText[i]);
        if (LastScan !=0)
            break;
    }
...
finalfade:
    ...
    if (LastScan == 0)
    {
        ...
        x = (screen_src_width - (int)(width * screen_src_minscale)) >> 1;
        y = (screen_src_height - (int)(height * screen_src_minscale)) >> 1;
        US_ClippedPrint(x, y - (6 * screen_src_minscale), NextGameString1);

        US_MeasureStr(&width, &height, "%s", NextGameString2);
        x = (screen_src_width - (int)(width * screen_src_minscale)) >> 1;
        y = (screen_src_height - (int)(height * screen_src_minscale)) >> 1;
        US_ClippedPrint(x, y + (6 * screen_src_minscale), NextGameString2);
...
void DrawTransmitterExplosions (void)
{
    ...
    for (i = 0; i < MAXTRANSMITTEREXPLOSIONS; i++)
    {
        DrawUnScaledSprite (Explosions[i].x, Explosions[i].y,
                            (W_GetNumForName(ExplosionInfo[Explosions[i].which].name) +
                             (Explosions[i].frame>>1)), 16);
    }
}
...
void ScrollString (int cy, char *string, byte *bkgnd, int scrolltime, int pausetime)
{
    ...
    x = (screen_src_width - (int)(width * screen_src_minscale)) >> 1;
    y = cy - ((int)(height * screen_src_minscale) >> 1);
    time1 = ((screen_src_height + (int)(20 * screen_src_minscale) - y) * scrolltime) /
            (screen_src_height + (int)(20 * screen_src_minscale) + (int)(height * screen_src_minscale));
    time2 = scrolltime - time1;

    WarpString(x, screen_src_height + (int)(10 * screen_src_minscale), x, y, time1, bkgnd, string);
    ...
    WarpString(x, y, x, (int)(-10 - height * screen_src_minscale), time2, bkgnd, string);
}
...
void DoBurningCastle (void)
{
    ...
    DrawNormalSpriteCinematicFull(W_GetNumForName("finale"), false);
    PrepareBackground(back);
    CurrentFont = smallfont;
    FlipPage();
    ...
    ScrollString(screen_src_height - (50 * screen_src_minscale),
                 &burnCastle1Msg[0], back, 4*VBLCOUNTER, 80);
...
void DoFailedScreen (void)
{
    ...
    ScrollString(screen_src_height >> 1, &notDoneMsg[0], back, 4*VBLCOUNTER, 100);
    VL_FadeOut(0, 255, 0, 0, 0, 80);
...
void DoTryAgainScreen (void)
{
    ...
    ScrollString(screen_src_height >> 1, tryAgainMsg, back, 4*VBLCOUNTER, 80);
    VL_FadeOut(0, 255, 0, 0, 0, 80);
...
void DestroyEarth (void)
{
    ...
    CalcTics();
    CalcTics();
    DrawNormalSpriteCinematicFull(W_GetNumForName("ourearth"), true);
    PrepareBackground(back);
...
void PlayerQuestionScreen (void)
{
    ...
    CurrentFont = (font_t *)W_CacheLumpName("newfnt1", PU_STATIC, Cvt_font_t, 1);
    ScrollString(screen_src_height >> 1, &doneMsg[0], back, 4*VBLCOUNTER, 40);
    ScrollString(screen_src_height >> 1, &youWin1Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic (W_GetNumForName("trilogo"));
    VL_DrawPic((screen_src_width >> 1) - (int)(152 * screen_src_minscale),
               (screen_src_height >> 1) - (int)(24 * screen_src_minscale),
               (pic_t *)W_CacheLumpNum(W_GetNumForName("player2"), PU_CACHE, Cvt_pic_t, 1));
    PrepareBackground(back);
    CurrentFont = smallfont;
    SD_Play(SD_PLAYERTBSND);
    ScrollString(screen_src_height >> 1, &youWin2Msg[0], back, 4*VBLCOUNTER, 100);

    VL_DrawPostPic (W_GetNumForName("trilogo"));
    VL_DrawPic((screen_src_width >> 1) - (int)(152 * screen_src_minscale),
               (screen_src_height >> 1) - (int)(24 * screen_src_minscale),
               (pic_t *)W_CacheLumpNum(W_GetNumForName("player1"), PU_CACHE, Cvt_pic_t, 1));
    PrepareBackground(back);
    SD_Play(SD_PLAYERTCSND);
    ScrollString(screen_src_height >> 1, &youWin3Msg[0], back, 4*VBLCOUNTER, 100);

    VL_DrawPostPic (W_GetNumForName("trilogo"));
    VL_DrawPic((screen_src_width >> 1) - (int)(152 * screen_src_minscale),
               (screen_src_height >> 1) - (int)(24 * screen_src_minscale),
               (pic_t *)W_CacheLumpNum(W_GetNumForName("player4"), PU_CACHE, Cvt_pic_t, 1));
    PrepareBackground(back);
    SD_Play(SD_PLAYERLNSND);
    ScrollString(screen_src_height >> 1, &youWin4Msg[0], back, 4*VBLCOUNTER, 100);

    VL_DrawPostPic (W_GetNumForName("trilogo"));
    VL_DrawPic((screen_src_width >> 1) - (int)(152 * screen_src_minscale),
               (screen_src_height >> 1) - (int)(24 * screen_src_minscale),
               (pic_t *)W_CacheLumpNum(W_GetNumForName("player3"), PU_CACHE, Cvt_pic_t, 1));
    PrepareBackground(back);
    SD_Play(SD_PLAYERDWSND);
    ScrollString(screen_src_height >> 1, &youWin5Msg[0], back, 4*VBLCOUNTER, 100);

    VL_DrawPostPic (W_GetNumForName("trilogo"));
    VL_DrawPic((screen_src_width >> 1) - (int)(152 * screen_src_minscale),
               (screen_src_height >> 1) - (int)(24 * screen_src_minscale),
               (pic_t *)W_CacheLumpNum(W_GetNumForName("player5"), PU_CACHE, Cvt_pic_t, 1));
    PrepareBackground(back);
    SD_Play(SD_PLAYERIPFSND);
    ScrollString(screen_src_height >> 1, &youWin6Msg[0], back, 4*VBLCOUNTER, 100);
...
void DoYouWin (void)
{
    ...
    CurrentFont = (font_t *)W_CacheLumpName("newfnt1", PU_STATIC, Cvt_font_t, 1);
    ScrollString(screen_src_height >> 1, &youWin7Msg[0], back, 4*VBLCOUNTER, 300);
    W_CacheLumpName("newfnt1", PU_CACHE, Cvt_font_t, 1);
    VL_FadeOut(0, 255, 0, 0, 0, 80);
...
void DoFinalEnd (void)
{
    ...
    CurrentFont = (font_t *)W_CacheLumpName("newfnt1", PU_STATIC, Cvt_font_t, 1);
    ScrollString( screen_src_height >> 1, &youWin8Msg[0], back, 4*VBLCOUNTER, 100);
    W_CacheLumpName("newfnt1", PU_CACHE, Cvt_font_t, 1);
    VL_FadeOut(0, 255, 0, 0, 0, 80);
...
void DIPCredits (void)
{
    ...
    CurrentFont = (font_t *)W_CacheLumpName ("newfnt1", PU_STATIC, Cvt_font_t, 1);
    ScrollString(screen_src_height >> 1, &creditsMsg[0], back, 4*VBLCOUNTER, 30);
    CurrentFont = smallfont;
    ScrollString(screen_src_height >> 1, &credits1Msg[0], back, 4*VBLCOUNTER, 50);
    ScrollString(screen_src_height >> 1, &credits2Msg[0], back, 4*VBLCOUNTER, 50);
    ScrollString(screen_src_height >> 1, &credits3Msg[0], back, 4*VBLCOUNTER, 50);
    ScrollString(screen_src_height >> 1, &credits4Msg[0], back, 4*VBLCOUNTER, 50);
    ScrollString(screen_src_height >> 1, &credits5Msg[0], back, 4*VBLCOUNTER, 50);
    ScrollString(screen_src_height >> 1, &credits6Msg[0], back, 4*VBLCOUNTER, 50);
    ScrollString(screen_src_height >> 1, &credits7Msg[0], back, 4*VBLCOUNTER, 50);
    ScrollString(screen_src_height >> 1, &credits8Msg[0], back, 4*VBLCOUNTER, 50);
    ScrollString(screen_src_height >> 1, &credits9Msg[0], back, 4*VBLCOUNTER, 50);
    ScrollString(screen_src_height >> 1, &credits10Msg[0], back, 4*VBLCOUNTER, 50);
    ScrollString(screen_src_height >> 1, &credits11Msg[0], back, 4*VBLCOUNTER, 80);
    ScrollString(screen_src_height >> 1, &credits12Msg[0], back, 4*VBLCOUNTER, 80);

    CurrentFont = (font_t *)W_CacheLumpName("newfnt1", PU_STATIC, Cvt_font_t, 1);
    ScrollString(screen_src_height >> 1, &actorsMsg[0], back, 4*VBLCOUNTER, 50);

    CurrentFont = smallfont;
    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("lwgshoo2"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &actors1Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("hg2shoo2"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &actors2Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("obpshoo1"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &actors3Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("ankshoo1"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &actors4Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("ligrise4"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &actors5Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("tritoss5"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &actors6Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("monkdr4"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &actors7Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("allksh4"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &actors8Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("robogrd1"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &actors9Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("darshoo1"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &actors10Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("hdope8"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &actors11Msg[0], back, 4*VBLCOUNTER, 50);
    ...
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &actors12Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("tomfly21"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &actors13Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    PrepareBackground ( back );
    CurrentFont = (font_t *)W_CacheLumpName ("newfnt1", PU_STATIC, Cvt_font_t, 1);
    ScrollString(screen_src_height >> 1, &playersCutMsg[0], back, 4*VBLCOUNTER, 40);

    CurrentFont = smallfont;
    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("cutmark"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &cut1Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("cutpat"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &cut2Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("cutmari"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &cut3Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("cutann"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &cut4Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("cutwill"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &cut5Msg[0], back, 4*VBLCOUNTER, 50);

    VL_DrawPostPic(W_GetNumForName("trilogo"));
    DrawNormalSprite(0, (200-128)>>1, W_GetNumForName("cutstev"));
    PrepareBackground(back);
    ScrollString(screen_src_height >> 1, &cut6Msg[0], back, 4*VBLCOUNTER, 50);
    ...
    ScrollString(screen_src_height - (25 * screen_src_minscale), &dipMsg[0], back, 4*VBLCOUNTER, 140);
    VL_FadeOut(0, 255, 0, 0, 0, 80);
...
void DoEndCinematicExtreme ( void )
{
    byte *bkgnd;
...
void DoInBetweenCinematic (int yoffset, int lump, int delay, boolean doFull, char *string)
{
    int width, height;
    int x, y;
    ...
    // TODO: FIXME: Change yoffset from post-scale to pre-scale. Change
    // DrawNormalSpriteFree to DrawNormalSprite.

    if (doFull)
        DrawNormalSpriteCinematicFull(lump, true);
    // If the aspect ratio is greater than 16:9, shrink it down to prevent too much clipping.
    else if (screen_src_width > (screen_src_height * 1.2) * 1.78)
        DrawNormalSpriteFree(screen_src_statusbar_x, yoffset, lump);
    else
        DrawNormalSpriteCinematic(yoffset, lump);

    CurrentFont = smallfont;
    US_MeasureStr(&width, &height, "%s", string);
    x = (screen_src_width - (width * screen_src_minscale)) / 2.0;
    y = screen_src_height - (10 * screen_src_hscale) - (height * screen_src_minscale);
...
void DrawPreviousCredits (int num, CreditType *Credits)
{
    ...
    for(i = 0; i < num; i++)
    {
        ...
        x = (screen_src_width - (width * screen_src_minscale)) / 2.0;
        y = Credits[i].endy * screen_src_minscale;
...
void WarpCreditString (int time, byte *back, int num, CreditType *Credits)
{
    ...
    x = (screen_src_width - (width * screen_src_minscale)) / 2.0;
    y = Credits[num].endy * screen_src_hscale;
    dy = ((y - (int)(CREDITSTARTY * screen_src_hscale)) << 16) / time;
    cy = (int)(CREDITSTARTY * screen_src_hscale) << 16;
    ...
    while (time > 0)
    {
        ...
        if (Credits[num].font == 0)
            ...
            CurrentFont = tinyfont;
        US_ClippedPrint(x, (cy >> 16) + (4 * screen_src_minscale), &Credits[num].text[0]);

        if (((cy >> 16) < (screen_src_height - (4 * screen_src_minscale))) && !soundplayed)
        {
            if (dopefish && SD_Started)
            {
                int snd;
...
void DoMicroStoryScreen (void)
{
    ...
    // Original: x=15, y=30
    x = (screen_src_width - (290 * screen_src_minscale)) / 2.0; // 320 - 15*2
    y = (screen_src_height - (140 * screen_src_minscale)) / 2.0; // 200 - 30*2
    ydelta = 9 * screen_src_minscale;
...
{
    double hScale = screen_src_hscale;
    int tempH = 0;
...
void RefreshClear (void)
{
    ...
    if (start > 0)
        VL_Bar(0, 0, screen_src_width, start, CEILINGCOLOR);
    else
        start = 0;
    ...
    if (start > 0)
        VL_Bar(0, base, screen_src_width, start, FLOORCOLOR);
}
