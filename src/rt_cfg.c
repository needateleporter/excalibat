...
unsigned long tempSBType = 1;
unsigned long tempSBPort = 0x220;
unsigned long tempSBIrq = 7;
unsigned long tempSBDma8 = 1;
unsigned long tempSBDma16 = 5;
unsigned long tempSBMidi = 0x330;
unsigned long tempSBEmu = 0x620;
...
boolean joystickenabled_old = 0;
boolean joypadenabled_old = 0;
...
static char *NewConfigName = "xcalibat.cfg";
static char NewScoresName[64] = "scores_other.rot";
static char *NewBattleName = "xcal-battle.cfg"; // TODO - Consolidate into xcalibat.cfg

#define NewScoresNameTemplate "scores-%s.rot"
#define NewSaveFileNameTemplate "rottgam?-%s.rot"

// Used only in "pure" mode.
static char *SoundName     = "SOUND.ROT";
static char *ConfigName    = "CONFIG.ROT";
static char *ScoresName    = "SCORES.ROT";
static char *SetupName     = "SETUP.ROT";
static char *SetupGameName = "ROTT.ROT";
...
void ReadNewScores (int rtl)
{
    int file;
    char filename[ 128 ];

    GenerateNewRTLBasedFileName(rtl, RTLBASED_SCORENAME);

    GetPathFromEnvironment(filename, ApogeePath, NewScoresName);
    ...
    {
        memcpy(&Scores, &DefaultScores, sizeof(DefaultScores));
    }
}

void GenerateNewRTLBasedFileName (int rtl, int filetype)
{
    char filename[64];
    ...
    if (enabledrtls != 0)
    {
        ...
        {
            case RTL_STANDARD:
                strcpy(filename, STANDARDGAMELEVELS);
                break;

            case RTL_SHAREWARE:
                strcpy(filename, SHAREWAREGAMELEVELS);
                break;

            case RTL_EXTREME:
                strcpy(filename, EXTREMEGAMELEVELS);
                break;
    ...
    filename_nopath = strrchr(filename, '/');
    fileext = strrchr(filename, '.');

    if (fileext == NULL)
        return;

    if (strcasecmp(fileext, ".RTL") == 0)
    {
        *fileext = '\0';

        if (filename_nopath == NULL)
        {
            filetemp = filename;

            while (*filetemp)
            {
                *filetemp = tolower(*filetemp);
                filetemp++;
            }

            switch (filetype)
            {
                case RTLBASED_SCORENAME:
                    sprintf(NewScoresName, NewScoresNameTemplate, filename);
                    break;

                case RTLBASED_SAVENAME:
                    sprintf(NewSaveFileName, NewSaveFileNameTemplate, filename);
                    break;
            }
        }
        else
        {
            ...
            {
                case RTLBASED_SCORENAME:
                    sprintf(NewScoresName, NewScoresNameTemplate, filename_nopath);
                    break;

                case RTLBASED_SAVENAME:
                    sprintf(NewSaveFileName, NewSaveFileNameTemplate, filename_nopath);
                    break;
            }
        }
    }
...
boolean ParseSoundFile (void)
{
    ...
    if (version == ROTTVERSION)
    {
        ...
        ReadInt ("MusicMode", &tempMusicMode);

        // Read in FX Mode

        ReadInt ("FXMode", &tempFXMode);

        // We don't have an accurate emulation of every single
        // soundcard, so for now we'll default to on/off.
        if (tempMusicMode > 0 || tempFXMode > 0)
        {
        ...
        ReadUnsigned ("SBType",  &tempSBType );
        ReadUnsigned ("SBPort",  &tempSBPort );
        ReadUnsigned ("SBIrq",   &tempSBIrq );
        ReadUnsigned ("SBDma8",  &tempSBDma8 );
        ReadUnsigned ("SBDma16", &tempSBDma16 );
        ReadUnsigned ("SBMidi",  &tempSBMidi );
        ReadUnsigned ("SBEmu",   &tempSBEmu );
    }
    else
        retval = false;
...
//   int temp;
    ...
    if (version == ROTTVERSION)
    {
        ...
        ReadBoolean("JoystickEnabled",&joystickenabled_old);

        // Read in JoypadEnabled
        ReadBoolean("JoypadEnabled",&joypadenabled_old);
...
void ParseNewConfigFile (void)
{
    // Video options
    ReadBoolean("Fullscreen", &screen_fullscreen);
    ReadInt("ScreenWidth", &screen_dest_width);
    ReadInt("ScreenHeight", &screen_dest_height);
    ReadBoolean("ClassicQuality", &screen_classic);
    ReadInt("CorrectAspectRatio", &screen_aspect);
    ...
    if (joystickenabled)
        IN_SetupJoy (JOY_TEMP, joyxmin, joyxmax, joyymin, joyymax);
}
...
void SetNewConfigDefaultValues (void)
{
    //
    //  no config file, so select default values
    //
...
void ReadNewConfig (void)
{
    char filename[ 128 ];

    GetPathFromEnvironment(filename, ApogeePath, NewConfigName);
    SetNewConfigDefaultValues();
    if (access(filename, F_OK) == 0)
    {
        LoadScriptFile(filename);
    ...
    GetPathFromEnvironment( filename, ApogeePath, NewBattleName );
    SetBattleDefaultValues ();
    if (access(filename,F_OK)==0)
    {
        LoadScriptFile(filename);
...
void CheckVendor (void)
{
    ...
    if (shareware)
        GetPathFromEnvironment(filename, ApogeePath, "VENDOR.DOC");
    else
        GetPathFromEnvironment(filename, ApogeePath, "LICENSE.DOC");
    ...
    if (access(filename, F_OK) == 0)
    {
        ...
        if (shareware)
            lump = W_GetNumForName("VENDOR");
        else
            lump = W_GetNumForName("LICENSE");
...
// TODO FIXME - Revert back to (void) with BattleName once the new version
//              is either created or consolidated into NewConfig.
void WriteBattleConfig (char *battleconfname)
{
    int  file;
    int  index;
    char filename[ 128 ];
    ...
    // Write Battle File
    GetPathFromEnvironment( filename, ApogeePath, battleconfname );
...
void WriteSoundConfig (void)
{
    ...
    WriteParameter(file,"MusicMode        ",tempMusicMode);
    ...
    WriteParameter(file,"FXMode           ",tempFXMode);
    ...
    WriteParameter(file, "SBType           ", tempSBType );
    WriteParameterHex(file, "SBPort           ", tempSBPort );
    WriteParameter(file, "SBIrq            ", tempSBIrq );
    WriteParameter(file, "SBDma8           ", tempSBDma8 );
    WriteParameter(file, "SBDma16          ", tempSBDma16 );
    WriteParameterHex(file, "SBMidi           ", tempSBMidi );
    WriteParameterHex(file, "SBEmu            ", tempSBEmu );
...
void WriteConfig (void)
{
    ...
    WriteParameter(file,"JoystickEnabled  ",joystickenabled_old);
    ...
    WriteParameter(file,"JoypadEnabled    ",joypadenabled_old);
...
void WriteNewConfig (void)
{
    ...
    WriteBattleConfig(NewBattleName);
    ...
    SafeWriteString(file, ";Rise of the Triad (XCalibat) Configuration File\n\n");

    SafeWriteString(file, "\n;--- Video Options\n");

    SafeWriteString(file, "\n;\n");
    SafeWriteString(file, "; 0 - Windowed mode\n");
    SafeWriteString(file, "; 1 - Fullscreen mode\n");
    WriteParameter (file, "Fullscreen         ", screen_fullscreen);

    SafeWriteString(file, "\n;\n");
    SafeWriteString(file, "; Screen Resolution (in pixels)\n");
    WriteParameter (file, "ScreenWidth        ", screen_dest_width);
    WriteParameter (file, "ScreenHeight       ", screen_dest_height);

    SafeWriteString(file, "\n;\n");
    SafeWriteString(file, "; 0 - (HIGH QUALITY)\n");
    SafeWriteString(file, "; 1 - (LOW QUALITY) Enforce original game's in-game\n");
    SafeWriteString(file, ";     resolution.\n");
    WriteParameter (file, "ClassicQuality     ", screen_classic);

    SafeWriteString(file, "\n;\n");
    SafeWriteString(file, "; -1 - [HQ] Ignore Pixel Aspect Ratio Correction.\n");
    SafeWriteString(file, ";      [CLASSIC] Stretches to full screen.\n");
    SafeWriteString(file, ";      (FASTEST)\n");
    SafeWriteString(file, ";  0 - [HQ) Ignore Pixel Aspect Ratio Correction.\n");
    SafeWriteString(file, ";      (CLASSIC] Letterboxes to 8:5.\n");
    SafeWriteString(file, ";  1 - [HQ] Enforce Pixel Aspect Ratio Correction.\n");
    SafeWriteString(file, ";      [CLASSIC] Letterboxes to 4:3.\n");
    SafeWriteString(file, ";      (MOST ACCURATE)\n");
    WriteParameter (file, "CorrectAspectRatio ", screen_aspect);
    ...
    SafeWriteString(file, "\n;\n");
    SafeWriteString(file, "; Resolution (8-bit or 16-bit)\n");
    WriteParameter (file, "NumBits       ", NumBits);
...
{
    // TODO LOWPRIORITY: Copy CODENAME and MACROS read/write to New Config for
    // use outside of pure mode and in-game saving of these values.
    ...
    GetPathFromEnvironment( filename, ApogeePath, SetupName );
    if (access (filename, F_OK) == 0)
    {
        LoadScriptFile (filename);
    ...
    GetPathFromEnvironment( filename, ApogeePath, SetupGameName );
    if (access (filename, F_OK) == 0)
    {
        LoadScriptFile (filename);
