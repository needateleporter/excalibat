    ...
    while ((ch = (unsigned char)*string++)!=0)
    {
        ...
        while (width--)
        {
            if ((x >= 0) && (x < screen_src_width))
            {
                ...
                while (height--)
                {
                    if ((y >= 0) && (y < screen_src_height))
                    {
                        if (*source > 0)
                            for (i = 0; i < xscalelookup[width]; i++)
                                for (j = 0; j < yscalelookup[height]; j++)
                                    *((byte *)(bufferofs + ylookup[y + j] + x + i)) = *source;
                    }

                    source++;
                    y += yscalelookup[height];
                }
            }
            x += xscalelookup[width];
        }
    }
}
    ...
    while (*s)
    {
        ...
        *se = '\0';
        ...
        if (c)
        {
            *se = c;
            s++;
            y += CurrentFont->height * screen_src_minscale;
        }
    }
}
        ...
        while (width--)
        {
            ...
            while (height--)
            {
                ...
                if (pix)
                    for (i = 0; i < xscalelookup[width]; i++)
                        for (j = 0; j < yscalelookup[height]; j++)
                            *(dest+i+(screen_src_width*j)) = pix;

                source++;
                dest += (screen_src_width * yscalelookup[height]);
            }

            px += xscalelookup[width];
            origdest += xscalelookup[width];
            dest = origdest;
        }
    }
    ...
    while (*s)
    {
        ...
        *se = '\0';
        ...
        if (c)
        {
            *se = c;
            s++;
            PrintY += CurrentFont->height * screen_src_minscale;
            PrintX = startx;
        }
    }
}
...
{
    VL_Bar (WindowX, WindowY, WindowW, WindowH, 13);
    PrintX = WindowX;
    PrintY = WindowY;
}
...
void US_DrawWindow (int x, int y, int w, int h)
{
    ...
    WindowX = x * 8;
    WindowY = y * 8;
    WindowW = w * 8 * screen_src_minscale;
    WindowH = h * 8 * screen_src_minscale;

    PrintX = WindowX * screen_src_minscale;
    PrintY = WindowY * screen_src_minscale;

    sx = (x - (1 * screen_src_minscale)) * 8;
    sy = (y - (1 * screen_src_minscale)) * 8;
    sw = (w + 1) * 8 * screen_src_minscale;
    sh = (h + 1) * 8 * screen_src_minscale;

    US_ClearWindow ();


    VL_DrawPic (sx, sy, Win1);

    VL_DrawPic (sx, sy + sh, Win7);

    for (i = sx + (8 * screen_src_minscale); i <= sx + sw - (8 * screen_src_minscale); i += (8 * screen_src_minscale))
    {
        VL_DrawPic (i, sy, Win2);
        VL_DrawPic (i, sy + sh, Win8);
    }

    VL_DrawPic (i, sy, Win3);
    VL_DrawPic (i, sy + sh, Win9);

    for (i = sy + (8 * screen_src_minscale); i <= sy + sh - (8 * screen_src_minscale); i += (8 * screen_src_minscale))
    {
        VL_DrawPic (sx, i, Win4);
        VL_DrawPic (sx + sw, i, Win6);
    }
}
...
void US_CenterWindow (int w, int h)
{
    ...
    US_DrawWindow (((screen_src_width / 8) - (w * screen_src_minscale)) / 2,
                   ((screen_src_height / 8) - (h * screen_src_minscale)) / 2,
                   w, h);
}
...
void DrawIntensityChar (char ch)
{
    ...
    while( width-- )
    {
        ...
        while( height-- )
        {
            ...
            if (pix != 0xFE)
            {
                col = GetIntensityColor(pix);

                for (i = 0; i < xscalelookup[width]; i++)
                    for (j = 0; j < yscalelookup[height]; j++)
                        *(dest+i+(screen_src_width*j)) = col;
            }
