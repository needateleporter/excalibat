...
void DrawRotPost (int height, byte *src, byte *buf, int origheight)
{
    ...
    y1 = (centery << 8) - (height << 5) + ((MENUOFFY << 8) * screen_src_minscale);
    y2 = (((height << 6) + y1) >> 8);

    if (((y1 >> 8) >= screen_src_height) || (y2 < 0))
        return;
    ...
    if (y2 > screen_src_height)
        DrawMenuPost(screen_src_height - y1, src, buf + ylookup[y1]);
    else
...
void GetPoint (int x1, int y1, int px, int py, int *screenx, int *height, int angle)
{
    ...
    *screenx = (int)(screen_src_width / 2.0) +
               ((ny * NORMALWIDTHMULTIPLIER * screen_src_minscale) / nx); // DEBUG: use assembly divide

    *height = (NORMALHEIGHTDIVISOR * screen_src_minscale / nx);
...
void InterpolatePlane (visobj_t *plane)
{
    ...
    {
        if ((i >= 0) && (i < viewwidth) && (posts[i].wallheight < (height >> 8)) && bot)
        {
...
{
#define PLANEX1 (-0x24000) // -144 << 10
#define PLANEX2 (0x23fff) // (144 << 10) - 1
#define PLANEW (16<<10)
    ...
    //menubuffers[0] = SafeMalloc(TEXTUREW * TEXTUREHEIGHT * screen_src_minscale * screen_src_minscale);
    //menubuffers[1] = SafeMalloc(TEXTUREW * TEXTUREHEIGHT * screen_src_minscale * screen_src_minscale);
...
void PositionMenuBuf (int angle, int distance, boolean drawbackground)
{
    ...
    if (drawbackground || !BackgroundDrawn)
    ...
    US_ClippedPrint ((int)(screen_src_width - (width * screen_src_minscale)) >> 1,
                     (int)((MENUTITLEY - titleyoffset) * screen_src_minscale), titlestring);
    CurrentFont = oldfont;
