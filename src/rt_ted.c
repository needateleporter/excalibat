...
boolean IsChristmas(void)
{
    ...
    return (((date.day == 24) || (date.day == 25)) && (date.month == 12));
}
...
void CheckHolidays(void)
{
    ...
    // TODO FIXME: None of these fit on a maxscaled mmbk background unless
    // it's 4:3. Add support for maxscaling and out-of-bounds rendering.
    //
    // Or just use pillarboxing/letterboxing for mmbk.
...
void DrawPreCache (void)
{
    if (!loadedgame)
    {
        ...
        if (BATTLEMODE)
        {
            int boxX = (screen_src_width - (260 * screen_src_minscale)) / 2.0;
            VL_DrawPostPic(W_GetNumForName("trilogo"));
            VL_TBar(boxX, (int)(23 * screen_src_minscale),
                    (int)(260 * screen_src_minscale), (int)(82 * screen_src_minscale));
            ShowBattleOptions(false, boxX + (26 * screen_src_minscale), 26 * screen_src_minscale);
        ...
        //int precachebarx = (screen_src_width - (265 * screen_src_minscale)) / 2.0;

        DrawNormalSprite(PRECACHEBARX, PRECACHEBARY, W_GetNumForName("cachebar"));

        CurrentFont = smallfont;

        PrintY = PRECACHEESTRINGY * screen_src_minscale;
        PrintX = PRECACHEESTRINGX * screen_src_minscale;
        ...
        VL_TBar((int)(PrintX - (2 * screen_src_minscale)), (int)(PrintY - (2 * screen_src_minscale)),
                (int)((width+4) * screen_src_minscale), (int)((height+4) * screen_src_minscale));
        US_BufPrint(&buf[0]);


        PrintY = PRECACHEASTRINGY * screen_src_minscale;

        memset(&buf[0], 0, sizeof(buf));
        memcpy(&buf[0], "AREA ", 5);

        if (!BATTLEMODE)
            itoa(GetLevel(gamestate.episode, gamestate.mapon), &buf[5], 10);
        else
            itoa(gamestate.mapon + 1, &buf[5], 10);

        US_MeasureStr(&width, &height, "%s", &buf[0]);
        PrintX = screen_src_width - ((20 + width) * screen_src_minscale);
        VL_TBar((int)(PrintX - (2 * screen_src_minscale)), (int)(PrintY - (2 * screen_src_minscale)),
                (int)((width+4) * screen_src_minscale), (int)((height+4) * screen_src_minscale));
        US_BufPrint(&buf[0]);


        PrintY = PRECACHESTRINGY * screen_src_minscale;
        ...
        PrintX = (screen_src_width - (width * screen_src_minscale)) / 2.0;
        PrintY = PRECACHESTRINGY * screen_src_minscale;
        VL_TBar((int)(PrintX - (2 * screen_src_minscale)), (int)(PrintY - (2 * screen_src_minscale)),
                (int)((width+4) * screen_src_minscale), (int)((height+4) * screen_src_minscale));
        US_BufPrint(&temp[0]);
...
void PreCache ( void )
{
    ...
    if (!loadedgame)
    {
        ...
        for (i = 1; i < cacheindex; i++)
        {
            ...
            while (lastmem <= currentmem)
            {
                DrawNormalSprite(PRECACHEBARX + PRECACHELED1X + ((lastmem<<2)),
                                 PRECACHEBARY + PRECACHELED1Y, W_GetNumForName ("led1"));
            ...
            while (lastcache <= currentcache)
            {
                DrawNormalSprite(PRECACHEBARX + PRECACHELED2X + ((lastcache<<2)),
                                 PRECACHEBARY + PRECACHELED2Y, W_GetNumForName ("led2"));
        ...
        // Force the "Press Any Key" prompt for single-player,
        // as on modern machines the loading screen can disappear immediately.
        // TODO - Make this optional in Bonus Options.
        if (BATTLEMODE || (!pure && !demoplayback))
        {
            int width,height;
            char buf[30];
            ...
            US_MeasureStr (&width, &height, "%s", &buf[0]);
            PrintX = (screen_src_width - (width * screen_src_minscale)) / 2;
            PrintY = 162*screen_src_hscale;
            VL_TBar((int)(PrintX - (2 * screen_src_minscale)), (int)(PrintY - (2 * screen_src_minscale)),
                    (int)((width+4) * screen_src_minscale), (int)((height+4) * screen_src_minscale));
            US_BufPrint(&buf[0]);
            VW_UpdateScreen();
...
void GetMapFileName (char *filename)
{
    if (BATTLEMODE)
    {
        if (BattleLevels.avail)
            strcpy(filename, BattleLevels.file);
        else if (GameLevels.avail)
            strcpy(filename, GameLevels.file);
        else if (shareware)
            strcpy(filename, DATADIR SHAREWAREBATTLELEVELS);
        else if (gamestate.Product == ROTT_SITELICENSE)
            strcpy(filename, DATADIR STANDARDBATTLELEVELS_TIER3);
        else if (gamestate.Product == ROTT_SUPERCD)
            strcpy(filename, DATADIR STANDARDBATTLELEVELS_TIER2);
        else
            strcpy(filename, DATADIR STANDARDBATTLELEVELS);
    }
    else
    {
        if (GameLevels.avail)
            ...
            strcpy(filename, DATADIR SHAREWAREGAMELEVELS);
        else
            strcpy(filename, DATADIR STANDARDGAMELEVELS);
    }
}

void GetMapFileNameForRTL (char *filename, int rtl)
{
    if (BATTLEMODE)
        return;
    else if (rtl == RTL_STANDARD)
        strcpy(filename, DATADIR STANDARDGAMELEVELS);
    else if (rtl == RTL_SHAREWARE)
        strcpy(filename, DATADIR SHAREWAREGAMELEVELS);
    else if (rtl == RTL_EXTREME)
        strcpy(filename, DATADIR EXTREMEGAMELEVELS);
    else if (rtl == RTL_CUSTOM && GameLevels.avail)
        strcpy(filename, GameLevels.file);
}
...

{
    ...
    if (BATTLEMODE || pure)
        ...
        GetMapFileNameForRTL(&filename[0], currentrtl);
...
void GetMapInfo (mapfileinfo_t *mapinfo)
{
    if (BATTLEMODE)
    {
        if (BattleLevels.avail)
            ...
            GetMapFileInfo(mapinfo, DATADIR SHAREWAREBATTLELEVELS);
        else if (gamestate.Product == ROTT_SITELICENSE)
            GetMapFileInfo(mapinfo, DATADIR STANDARDBATTLELEVELS_TIER3);
        else if (gamestate.Product == ROTT_SUPERCD)
            GetMapFileInfo(mapinfo, DATADIR STANDARDBATTLELEVELS_TIER2);
        else
            GetMapFileInfo(mapinfo, DATADIR STANDARDBATTLELEVELS);
    }
    else
    {
        if (GameLevels.avail)
            ...
            GetMapFileInfo(mapinfo, DATADIR SHAREWAREGAMELEVELS);
        else
            GetMapFileInfo(mapinfo, DATADIR STANDARDGAMELEVELS);
    }
}

void GetMapInfoForRTL (mapfileinfo_t *mapinfo, int rtl)
{
    if (BATTLEMODE)
        return;
    else if (rtl == RTL_STANDARD)
        ...
        GetMapFileInfo(mapinfo, DATADIR EXTREMEGAMELEVELS);
    else if (rtl == RTL_CUSTOM && GameLevels.avail)
        GetAlternateMapInfo(mapinfo, &GameLevels);
}
...
void LoadROTTMap (int mapnum)
{
    if (BATTLEMODE)
    {
        if (BattleLevels.avail)
            ...
            ReadROTTMap(DATADIR SHAREWAREBATTLELEVELS, mapnum);
        else if (gamestate.Product == ROTT_SITELICENSE)
            ReadROTTMap(DATADIR STANDARDBATTLELEVELS_TIER3, mapnum);
        else if (gamestate.Product == ROTT_SUPERCD)
            ReadROTTMap(DATADIR STANDARDBATTLELEVELS_TIER2, mapnum);
        else
            ReadROTTMap(DATADIR STANDARDBATTLELEVELS, mapnum);
    }
    else
    {
        if (GameLevels.avail)
            ...
            ReadROTTMap(DATADIR SHAREWAREGAMELEVELS, mapnum);
        else
            ReadROTTMap(DATADIR STANDARDGAMELEVELS, mapnum);
    }
}

void LoadROTTMapForRTL (int mapnum, int rtl)
{
    if (BATTLEMODE)
        return;
    else if (rtl == RTL_STANDARD)
        ...
        ReadROTTMap(DATADIR EXTREMEGAMELEVELS, mapnum);
    else if (rtl == RTL_CUSTOM && GameLevels.avail)
        LoadAlternateMap(&GameLevels, mapnum);
}
    ...
    for(j=0; j<mapheight; j++)
    {
        for(i=0; i<mapwidth; i++)
        {
            ...
            switch(tile)
            {
                ...
                case 460:
                    if ( useWind && gamestate.Product != ROTT_SHAREWARE )
    ...
    for (j = 0; j < mapheight; j++)
    {
        for (i = 0; i < mapwidth; i++)
        {
            ...
            switch (*map)
            {
                ...
                case 94:   // Fix Gold Door
                    *map = 101;
                    *(&(mapplanes[1][(MAPSIZE * j) + i]))=29;
                    break;

                case 95:   // Fix Silver Door
                    *map = 101;
                    *(&(mapplanes[1][(MAPSIZE * j) + i]))=30;
                    break;

                case 96:   // Fix Iron Door
                    *map = 101;
                    *(&(mapplanes[1][(MAPSIZE * j) + i]))=31;
                    break;

                case 97:   // Fix Oscuro Door
                    *map = 101;
                    *(&(mapplanes[1][(MAPSIZE * j) + i]))=32;
                    break;

                case 161:   // Tall pillar
                    *map = 0;
                    break;
    ...
    for (j = 0; j < mapheight; j++)
    {
        for (i = 0; i < mapwidth; i++)
        {
            ...
            switch (*map)
            {
                ...
                case 55:  // Split Missile -> Heatseeker
                    *map = 52;
                    break;

                case 56:  // KES -> Drunk Missile
                    *map = 53;
                    break;

                case 253:  // Dog Mode -> Mercury Mode
                    *map = 254;
                    break;

                case 262:  // Tom Larva -> Collector Item
                    *map = 263;
                    break;
            }
        }
...
========================================
=
= DoRegisteredConversionBackgroundPlane
=
========================================
*/
void DoRegisteredConversionBackgroundPlane (void)
{
    int i, j;
    word *map;
    ...
    for (j = 0; j < mapheight; j++)
    {
        for (i = 0; i < mapwidth; i++)
        {
            ...
            switch (*map)
            {
                ...
                case 232: // Chain belt -> Machinery
                    *map = 231;
                    break;

                case 228: // Grey waterfall -> Blue waterfall
                    *map = 230;
                    break;
...
==================
=
= DoRegisteredConversion
=
==================
*/
void DoRegisteredConversion (void)
{
    DoRegisteredConversionBackgroundPlane();
}
...
void SetupGameLevel (void)
{
    ...
    if (BATTLEMODE || pure)
        ...
        LoadROTTMapForRTL(gamestate.mapon, currentrtl);
