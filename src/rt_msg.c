...
void DeletePriorityMessage (int flags)
{
    ...
    for (i = 0; i < MAXMSGS; i++)
        if (Messages[i].active == 1 && Messages[i].flags == flags)
            DeleteMessage(i);
}
...
int GetFreeMessage ()
{
    ...
    for (i = 0; i < MAXMSGS; i++)
        if (Messages[i].tictime >= 0)
            if ((found == -1) ||
                    (Messages[i].tictime < Messages[found].tictime))
                found = i;
...
====================
*/
void SetMessage (int num, char *text, int flags)
{
    int i;
    int msg;
    int length;
    ...
    {
        int l = COM_MAXTEXTSTRINGLENGTH + 1;
...
{
    int i;
    int messagetics = GetTicCount() - LastMessageTime;
...
{
    PrintX = 1 * screen_src_minscale;
    PrintY = (2 + (position * 9)) * screen_src_minscale;

    if (SHOW_TOP_STATUS_BAR())
        PrintY += 16 * screen_src_minscale;
    ...
    switch (Messages[num].flags)
    {
        ...
        case MSG_MODEM:
            ...
            PrintX += (8 * 8) * screen_src_minscale;
        ...
        for (i = 0; i < MAXMSGS; i++)
        {
            if (EraseMessage[i])
            {
                ...
                if (viewsize < 15)
                {
                    ...
                    DrawTiledRegion(0, y * screen_src_minscale, screen_src_width, 9 * screen_src_minscale, 0, y, shape);
                }
