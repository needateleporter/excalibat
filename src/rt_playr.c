...
{
    if (gamestate.SpawnEluder && !shareware)
    {
    ...
    else if (gamestate.PlayerHasGun[pstate - &PLAYERSTATE[0]])
    {
...
void SpawnGunSmoke(int x, int y, int z, int angle, int bullethole)
{
    ...
    else if ((angle < (15*ANGLES/16)) && (angle > (13*ANGLES/16)))
        chance = 128;
    else
        chance = 20;
...
//      statobj_t*      tempsprite=NULL;
    ...
    else if ((ob->flags & FL_DESIGNATED) && (BATTLEMODE) && (gamestate.battlemode == battle_Tag))
    {
        NewState(ob,&s_tag);
        //return;
    }
...
void PollMove (void)
{
    ...
    if (player->flags & FL_FLEET && !flappy)
        y += y>>1;
...
keys:
        ...
        case stat_twopistol:
            if (!GiveBulletWeapon(ob, wp_twopistol, check))
                return;
            LocalBonusMessage("You got an extra pistol.");
            break;

        case stat_mp40:
            if (!GiveBulletWeapon(ob, wp_mp40, check))
                return;
            LocalBonusMessage("You picked up an MP40.");
            break;

        case stat_bazooka:
            if (!GivePlayerMissileWeapon(ob, pstate, check))
                return;
        ...
        case stat_fleetfeet:
            ...
            if (flappy)
            {
                LocalBonusMessage("Mercury Mode! Press USE key to fly.");
            }
            else
            {
...
void Thrust ( objtype *ob )
{
    ...
    if (index && (abs(ob->z - nominalheight) < 5))
    {
        if (!TRIGGER[index-1])
        {
            ...
            {
                // DONT ALLOW BAD touchplate ( == 0 ) see rt_door.c
#endif
...
void PlayerTiltHead(objtype *ob)
{
    ...
    else if (pstate->guntarget)
    {
        ...
        yzangle = atan2_appx(xydist,dz) + HORIZONYZOFFSET;
        Fix(yzangle);
    ...
    else
    {
        ...
        {
            if (!(ob->flags & FL_FLEET && !flappy))
            {
                dyz = pstate->buttonstate[bt_lookdown] ? 0 : YZTILTSPEED;
                SetNormalHorizon(ob);
            }
        }
        else if (pstate->buttonstate[bt_lookdown])
        {
            if (!(ob->flags & FL_FLEET && !flappy))
            {
                dyz = -YZTILTSPEED;
        ...
        if (!(ob->flags & FL_DOGMODE) && !(ob->flags & FL_GODMODE) && !(ob->flags & FL_FLEET) &&
                !(ob->flags&FL_RIDING) && (ob->momentumz > (GRAVITY << 1)))
        {
...
//   int     i;
    ...
    if (godmode && Keyboard[sc_Insert])
    {
        ...
        if ((shareware && (pstate->weapon == MAXWEAPONS_SW-1)) ||
                (pstate->weapon == MAXWEAPONS-1))
            pstate->new_weapon = 0;
        else
            pstate->new_weapon ++;
...
void CheckFlying(objtype *ob, playertype *pstate)
{
    ...
    else if (!M_ISDOOR(((ob->x + costable[ob->angle]) >> 16),
...
void T_Player (objtype *ob)
{
    ...
    int eluder = (pstate->weapon == wp_dog) && gamestate.SpawnEluder;
    ...
    if (!(ob->flags & FL_PAIN))
    {
        ...
        else if (NOMOM && (ob->state != &s_dogwait))
    ...
    if (ARMED(ob->dirchoosetime))   //(gamestate.battlemode != battle_Tag))
    {
        if (pstate->weapondowntics == 1)    // change to up; during change, up and down
        {
            pstate->weapondowntics = 0;     // are never zero at the same time
            pstate->weaponframe = pstate->attackframe = 0;
...
//   if ( pstate->buttonstate[bt_use] && (!W_CHANGE(pstate)) )
    ...
    if ((!ARMED(ob->dirchoosetime)) && (pstate->weapon != wp_dog))
        return;
    ...
    if (pstate->buttonstate[bt_attack])
    {
        if (eluder)
            Cmd_Fire(ob);
        else if ((pstate->weapon == wp_bat) || (pstate->weapon == wp_dog))
        {
            ...
            else if ((pstate->batblast>>4) != (oldblast>>4))
            {
                int handle = SD_PlaySoundRTP(SD_DOGMODEPREPBLASTSND, ob->x, ob->y);
        ...
        if ((pstate->weapon != wp_split) || (!pstate->buttonheld[bt_attack]))
        {
            ...
            Cmd_Fire (ob);
              ...
              ((pstate->weapon == wp_dog) && (!eluder))) && (pstate->buttonheld[bt_attack]))
    {
        if (pstate->weapon == wp_bat)
