...
{
    if (PLAYER[0]->flags & FL_DOGMODE || shareware)
        return;
...
void RecordDemoQuery (void)
{
    ...
    {
        level = ParseNum(str);
        if ((level > 0) && (((level < 37) && !shareware) || (level < 9)))
        {
        ...
        switch (which)
        {
            ...
            case FLAPPY:
                if (!pure)
                {
                    ...
                    if (PLAYER[0]->flags & FL_FLEET)
                    {
                        PLAYER[0]->flags &= ~(FL_FLEET);
                        locplayerstate->poweruptime = 0;
                        GM_UpdateBonus (0, true);
                    }

                    if (flappy)
                        AddMessage("Pre-release Mercury Mode on!", MSG_GAME);
                    else
                        AddMessage("Pre-release Mercury Mode off!", MSG_GAME);
                }
                break;
        }
    }
...
void CheckDebug (void)
{
    ...
    if (!DebugOk)
    {
        CheckCode(ENABLECHEAT);     // Check for Debug switch only
        CheckCode(ENABLECHEATALT);  // Check for Debug switch only
        CheckCode(SOMEITEMS);       // Check for Slacker pack
        CheckCode(SOMEITEMSALT);    // Check for Slacker pack
    }
    else
    {
        if (demoplayback)
