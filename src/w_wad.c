...
void W_CheckWADIntegrity (void)
{
    ...
    // TODO: This check is returning different values for each game init.
    // Files are not being added correctly in W_AddFile?
    crc = CalculateCRC((byte *)lumpinfo, numlumps * sizeof(lumpinfo_t));

    if (shareware_deluxe)
        crccheck = WADCHECKSUM_DELUXE;
    else if (shareware)
        crccheck = WADCHECKSUM_HUNTBGIN;
    else
        crccheck = WADCHECKSUM_DARKWAR;
