...
int SD_Startup ( boolean bombonerror )
{
    ...
    soundtype = shareware ? fx_digital_sw : fx_digital;

    if ( soundtype == fx_digital || soundtype == fx_digital_sw)
    {
        if ( SoundsRemapped == false )
        {
            for( i = 0; i < SD_LASTSOUND; i++ )
            {
                ...
                snd = sounds[ i ].snds[ soundtype ];
                if ( snd >= 0)
                {
                    sounds[ i ].snds[ soundtype ] = W_GetNumForName(
    ...
    status=FX_Init( voices, channels, bits, 11000 );
...
#define MAXSONGS_SW 18
static song_t rottsongssw[MAXSONGS_SW] =
{
    { loop_no,  song_apogee  ,"FANFARE2","Apogee Fanfare"},
    { loop_yes, song_title   ,"RISE",    "Rise"},
    { loop_yes, song_menu    ,"MMMENU",  "MMMenu"},
...
{
    if ((MusicMode > 0) && MU_Started)
    {
        SetMenuHeader(shareware ?
                      rottsongssw[which].songname :
                      rottsongs[which].songname);
        MU_PlaySong(which);
    }
}
...
void MU_JukeBoxRedraw (void)
{
    if ((MusicMode > 0) && MU_Started)
        SetMenuHeader(shareware ?
                      rottsongssw[lastsongnumber].songname :
                      rottsongs[lastsongnumber].songname);
}
...
{
    char *SongNames[MAXSONGS], *SongNamesSW[MAXSONGS_SW];
    int i;

    if (shareware)
    {
        for(i = 0; i < MAXSONGS_SW; i++)
        {
            SongNamesSW[i] = rottsongssw[i].songname;
        }

        HandleMultiPageCustomMenu(SongNamesSW, MAXSONGS_SW, lastsongnumber,
                                  "Jukebox", MU_PlayJukeBoxSong, MU_JukeBoxRedraw, false);

        if (rottsongssw[lastsongnumber].loopflag == loop_no)
...
int MU_Startup (boolean bombonerror)
{
    ...
    if (!MusicMode) // Check if it is off
        return 0;
...
int MU_GetNumForType (int type)
{
    ...
    if (shareware)
    {
        for (i = 0; i < MAXSONGS_SW; i++)
        {
            if (rottsongssw[i].songtype == type)
                return i;
        }
    }
    else
...
void MU_PlaySong (int num)
{
    ...
    if ((shareware && (num >= MAXSONGS_SW)) ||
            (!shareware && (num >= MAXSONGS)))
    ...
    if (shareware)
        lump = W_GetNumForName(rottsongssw[num].lumpname);
    else
    ...
    if ((shareware && rottsongssw[num].loopflag == loop_yes) ||
            (!shareware && rottsongs[num].loopflag == loop_yes))
        MUSIC_PlaySong(currentsong, size, MUSIC_LoopSong);
    else
        MUSIC_PlaySong(currentsong, size, MUSIC_PlayOnce);

    //MUSIC_PlaySongFromExternalFile("test.ogg", MUSIC_LoopSong);
...
void MU_StopSong (void)
{
    ...
    {
        if (shareware)
            W_CacheLumpName(rottsongssw[lastsongnumber].lumpname,PU_CACHE, CvtNull, 1);
        else
...
void MU_SaveMusic (byte **buf, int *size)
{
    ...
    *buf = (byte *) SafeMalloc(*size);
    ...
    if ((shareware && rottsongssw[i].songtype == song_menu) ||
            (!shareware && rottsongs[i].songtype == song_menu))
    {
