                    ...
                    )
{
    ...
    else if (tcl == patrolgunobj)
    {
        ...
        if (victim->hitpoints <= 0)
        {
            ...
            else if (!shareware)  // this is normal
...
// bosses are "special" ==========================
    ...
    else if ((tcl == b_darksnakeobj) && (victim->temp3) && !shareware) // red snake
    {
...
void SpawnSuperFatalityGibs(objtype *ob,objtype *attacker)
{
...

    {
        ...
        else
        {
            ...
            // TODO HIGHPRIORITY: The gib system is inaccurate.
            // Might have been a 1.DFISH change from the watermarks
            // from MED. Need to do a deep dive on the 1.DFISH patch.
    ...
    if ((ob->flags & FL_HBM) && (gamestate.violence >= vl_high))
    {
        ...
        if (gamestate.violence == vl_excessive)
        {
               ...
               )
            {
                ...
                // TODO HIGHPRIORITY: Inaccurate gibs. See SpawnSuperFatalityGibs
...
//      head->obclass = inertobj;
    ...
    else if ((ob->state == ob->state->next) &&
             (ob->flags & FL_DYING)
            )
    {
        KillActor(ob);
...
#define MINSTATZDIFF 58
                    ...
                    if ((ocl == b_darksnakeobj) && (tempstat->itemnumber == stat_heatgrate) && !shareware)
                    {
...
void T_Chase (objtype *ob)
{
    ...
    if ((ocl == deathmonkobj) || (ocl == blitzguardobj))
    {
        ...
        if ((dx < TOUCHDIST) && (dy < TOUCHDIST) && (dz < (TOUCHDIST >> 10)))
        {
            ...
            else if ((!ob->temp3) && (PLAYERSTATE[0].missileweapon != -1) &&
